...
type PatientDataRepository interface {
	...
	GetDataV1(ctx context.Context, traceID string, userID string, dates *schema.Date, excludeTypes []string) (goComMgo.StorageIterator, error)
	...
	GetLoopMode(ctx context.Context, traceID string, userID string, dates *schema.Date) ([]schema.LoopModeEvent, error)
}
